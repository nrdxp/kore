...
let
  ...
  haskell-nix = import sources."haskell.nix" { inherit (import sources.nixpkgs { inherit system; }) pkgs; };
  pkgs = import haskell-nix.sources.nixpkgs-unstable ({ inherit system; } // haskell-nix.nixpkgsArgs);
  inherit (pkgs) lib;
